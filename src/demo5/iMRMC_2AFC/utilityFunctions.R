...
createIMRMCdf <- function(
  ...
  data0 <- data.frame(
    ...
    caseID     = as.character(data0[ , caseID]),
  ...
  data1 <- data.frame(
    ...
    caseID     = as.character(data1[ , caseID]),
...
## createIMRMC2AFCdf ####
#' Convert a data frame with all needed factors to doIMRMC formatted data frame
#' This is modified from createIMRMCdf especially for 2AFC study output files generated from WashU MIR web-based app.
#'
#' @param dFrame This data frame includes columns for readerID, caseID, modalityID, score, and truth.
#'        These columns are not expected to be named as such and other columns may exist.
#' @param keyColumns This list identifies the column names of the data frame to be used for the analysis.
...
#'
# @examples
createIMRMC2AFCdf <- function(
    dFrame,
    keyColumns = list(readerID = "readerID", caseID = "caseID", modalityID = "modalityID", score = "score")
){
  ...
  data0 <- data.frame(
    readerID   = data0[ , readerID],
    caseID     = paste("fake",as.character(data0[ ,caseID]), sep=""),
    modalityID = modalityID,
    score      = 0.5, # fake cases: https://github.com/DIDSR/iMRMC/wiki/MRMC-analysis-of-binary-data
    #score      = 1 - data0[ , score],
    stringsAsFactors = TRUE
  )
  data1 <- dFrame
  data1 <- data.frame(
    readerID   = data1[ , readerID],
    caseID     = paste("real",as.character(data1[ , caseID]), sep=""),
    modalityID = modalityID,
    score      = data1[ , score],
    stringsAsFactors = TRUE
  )
...
createIMRMC2AFCdf_v2 <- function(
  ...
  data0 <- dFrame
  data0 <- data.frame(
    readerID   = unique(data0[,readerID]),#data0[ , readerID],
    caseID     = "fake1",#paste("fake",as.character(data0[ ,caseID]), sep=""),
    modalityID = modalityID,
    score      = 0.5, # fake cases: https://github.com/DIDSR/iMRMC/wiki/MRMC-analysis-of-binary-data
    #score      = 1 - data0[ , score],
    stringsAsFactors = TRUE
