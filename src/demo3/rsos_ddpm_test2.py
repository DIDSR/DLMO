...
print('Data range ([min, max]) in testing data: [%.4f, %.4f]' % (np.min(testing_data), np.max(testing_data)))
...
for acc in [acceleration]:
    ...
    if acc > 1:
        ...
        if demo: utils.plot2dlayers(np.abs(mask), title='Possion disk-based \n subsampling pattern for '+str(acc)+'x', colorbar=True)
