...
# primarily because the inference code from line 203
# on-wards is not multi-gpu adjusted for different
# processors working on different MR batches.
# ****************************************************
'''
import torch
import numpy as np
...
parser.add_argument('--pretrained-model-epoch', type=int, default=150, help='Transfered learning ' \
...
pretrained_model_checkpoint_format = args.pretrained_model_checkpoint_format.format(model_name=args.model_name, \
...
with tqdm(total=len(test_loader),
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        # i = i + 1
        t.update(1)
        print('rank:', hvd.rank(), 'outputs:', np.sum(np.sum(preds, axis=2), axis=1))
