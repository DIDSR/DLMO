...
4. [*DLMO testing*](https://github.com/DIDSR/DLMO/tree/main/src/demo5/DLMO_test)
   ...
   ```
   python dlmo_test.py --task rayleigh --test-path ../../demo3/rsos_rec --acceleration 1 \
   --batch-size 10 --pretrained-model-path trained_model/mri_cnn_dlmo_acc_1_hvd --pretrained-model-epoch 170
   ```

   #### Process the DLMO discrimination-based outputs from the three reconstructions above and compute PC (AUC) by signal length (4–8 mm) to generate a summary plot, as shown in Figure 4 of our [DLMO paper](https://arxiv.org/abs/2602.22535):
